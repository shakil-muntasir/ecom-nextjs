...
const HomePage = () => {
  ...
  useEffect(() => {
    if (!selectedCategory) {
      ...
      axios.get(`/api/categories/${selectedCategory.id}/products`)
        .then(response => {
          setProducts(response.data);
        })
        .catch(error => {
  ...
  return (
    <main>
      <nav className="bg-white h-17 border-b-2 border-gray-200">
        <div className="container mx-auto py-4 flex justify-between items-center">
          ...
          <div className="ml-auto space-x-4 flex items-center">
            <Link href="/login">
              <button className="text-[#F47458] text-orange px-3 py-2 rounded-md font-semibold hover:bg-gray-200">
                Login
              </button>
            </Link>
            <Link href="/signup">
      ...
      <div className="container mt-4 ml-4 ">
        <select
          onChange={(e) => handleCategoryChange(JSON.parse(e.target.value))}
          className="bg-white border-2 border-[#F47458] rounded-md py-2 px-4"
        >
